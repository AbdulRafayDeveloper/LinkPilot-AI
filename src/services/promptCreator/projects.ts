...
type StoredProject = Pick<IPromptProject, "name" | "instructions"> & { _id: { toString: () => string } }
...
export async function createProject(viewer: Viewer, input: { name: string; instructions: string }): Promise<PromptProject> {
  ...
  return toProject(record.toObject() as unknown as StoredProject, 0)
}
...
export async function projectForPrompt(viewer: Viewer, projectId: string | null): Promise<PromptProject | null> {
  ...
  await connectDatabase()
  const record = (await PromptProjectModel.findOne(filter, { name: 1, instructions: 1 }).lean()) as unknown as StoredProject | null
