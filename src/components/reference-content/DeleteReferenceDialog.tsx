...
/**
 * Asks before deleting one saved item, naming it so the right one is going. Nothing is deleted
 * until Delete is pressed; Escape, the backdrop and Cancel all close it without deleting.
 */
export const DeleteReferenceDialog: React.FC<DeleteReferenceDialogProps> = ({ item, isDeleting, error, onConfirm, onClose }) => {
  const cancelRef = useRef<HTMLButtonElement>(null)

  return (
    <Modal
      title="Delete this saved content?"
      description={`"${item.title}" will be deleted for good. It cannot be undone.`}
      onClose={onClose}
    ...
    >
      <p className="text-sm leading-relaxed text-on-surface-variant">
        Copy anything you still need from it first. Nothing else in the app is touched.
      </p>
    </Modal>
  )
}
