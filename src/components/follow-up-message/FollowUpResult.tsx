...
export const FollowUpResult: React.FC<FollowUpResultProps> = ({ status, result, error, onRetry }) => {
  // The message as the user edits it; copy and the count follow the edits
  const message = useEditableText("follow-up-message", result?.message ?? "")
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2 min-h-[26px]">
        ...
        {status === "success" && result && <CopyButton text={message.value} label="Copy follow-up message" showLabel />}
      </div>
      ...
      {status === "success" && result && (
        <div className="flex-1 min-h-0 flex flex-col gap-3">
          <EditableOutput text={message} label="Follow-up message" />
